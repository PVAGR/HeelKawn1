...
const TICK_INTERVAL_SECONDS: float = 0.005

## Length of the **visual** day/night cycle in ticks (also used for "Day N" in HUD).
## At 1x this is TICKS_PER_VISUAL_DAY * TICK_INTERVAL_SECONDS real seconds (600 * 0.005 = 3s).
const TICKS_PER_VISUAL_DAY: int = 600

## In-world **year** for milestones, exports, and kernel diagnostics (~30k ticks).
## Wall time at 1x: TICKS_PER_SIM_YEAR * TICK_INTERVAL_SECONDS (30000 * 0.005 = 150s ≈ 2.5 min).
