...
func sync_from_settlements() -> void:
	...
	if SettlementMemory != null and SettlementMemory.has_method("get_formal_settlements"):
		for st_any in SettlementMemory.get_formal_settlements():
			...
			if ckr >= 0:
				formal_zones[str(ckr)] = true
	var stale_zones: Array = []
	for zid in _house_by_zone.keys():
		if not formal_zones.has(str(zid)):
			stale_zones.append(zid)
	for zid in stale_zones:
		_house_by_zone.erase(zid)
	if formal_zones.is_empty():
		return
	for st_any in SettlementMemory.get_formal_settlements():
		if not (st_any is Dictionary):
			continue
