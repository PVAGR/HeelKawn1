...
- **Determinism mechanism**:
  ...
  - Uses purely deterministic formulas in `spawn_generational_pawn` (`data.age = 20 + tick_seed % 5`, etc.)
- **Used by**:
...
- **Non-obvious design meaning**:
  ...
  - The selection pipeline is shaped by an **obedience weight** from `WorldAI` if `WorldAI` exists.
    - This means AI integration is meant to steer labor compliance without rewriting job posting/validation logic.
...
- **Responsibility**:
  ...
  - Implements deterministic personality → neural network initialization based on `WorldRNG` streams.
...
1. **Bootstrap / generation**
   ...
   2) `Main._bootstrap_colony()`:
      ...
      - spawns animals and registers them into world meta.

2. **Tick loop**
   1) `GameManager._process()` advances accumulator and emits `game_tick` with `tick_count`.
   2) `Main._on_game_tick(tick)` runs:
      - player intent dispatch (`PlayerIntentQueue`)
      - meaning ambiance controller tick
      ...
      - planning updates: settlement + trade at rate-limited intervals
      - rebirth and settlement architect passes at periodic boundaries
      - mining-react incremental scanning with a work budget

3. **Job lifecycle and fact recording**
   1) Systems post jobs into `JobManager` (e.g., `Main._seed_jobs_from_world`, `Main._react_to_mining_progress_step`).
...
2. **Expensive work is intervalized and speed-adaptive**
   - Main uses many `*_interval_for_speed` helpers and tick modulus gating.
