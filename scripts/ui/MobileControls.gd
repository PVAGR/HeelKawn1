...
func _build_ui() -> void:
	...
	_add_button(grid, "26x", _on_speed_26_pressed)
	...
	_add_button(grid, "Focus", _on_focus_pressed)
	...
	_add_button(grid, "Mode", _on_mode_pressed)
...
func _on_speed_26_pressed() -> void:
...
func _on_focus_pressed() -> void:
	if _main == null or _camera == null:
		return
	var selected_v: Variant = _main.get("_selected_pawn")
	if selected_v is Node2D and is_instance_valid(selected_v):
		_camera.global_position = (selected_v as Node2D).global_position
		return
	var player_v: Variant = _main.get("_player_pawn")
	if player_v is Node2D and is_instance_valid(player_v):
		_camera.global_position = (player_v as Node2D).global_position
...
func _on_mode_pressed() -> void:
	if _main != null and _main.has_method("_toggle_incarnation_mode"):
		_main.call("_toggle_incarnation_mode")
