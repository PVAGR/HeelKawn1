			...
			if typeof(pdata) == TYPE_DICTIONARY:
				...
				if pdata != null:
					if pdata.has_method("get"):
						...
						pid = int(pdata.id)
			var lvl: int = 0
			if FactionManager != null and FactionManager.has_method("get_authority_level"):
				lvl = int(FactionManager.get_authority_level(pid, FactionManager.AuthorityContext.CIVIL))
			if lvl >= 3:
				leaders.append(pid)
			elif lvl == 2:
