...
func visual_truth_snapshot() -> Dictionary:
	...
	if area != null and is_instance_valid(area):
		for child in area.get_children():
			if child is CollisionShape2D:
				var cs: CollisionShape2D = child as CollisionShape2D
				shape_disabled = cs.disabled
				shape_non_null = cs.shape != null
				if not cs.disabled and cs.shape != null:
					shape_ok = true
					break
	var sprite_path: String = ""
	var sprite_parent_path: String = ""
	if sprite != null and is_instance_valid(sprite):
		sprite_path = str(sprite.get_path())
		if sprite.get_parent() != null:
			sprite_parent_path = str(sprite.get_parent().get_path())
	
	var click_area_parent_path: String = ""
	if area != null and is_instance_valid(area) and area.get_parent() != null:
		click_area_parent_path = str(area.get_parent().get_path())
	
	var selection_capable: bool = false
	var failure_reason: String = ""
	
	if sprite != null and is_instance_valid(sprite):
		if sprite.texture == null:
			failure_reason = "texture_null"
		elif not (visible and is_visible_in_tree() and sprite.is_visible_in_tree()):
			failure_reason = "not_visible"
		elif not has_valid_world_position():
			failure_reason = "invalid_position"
		elif area == null or not is_instance_valid(area):
			failure_reason = "click_area_missing"
		elif not area.input_pickable:
			failure_reason = "input_pickable_false"
		elif area.collision_layer == 0:
			failure_reason = "collision_layer_zero"
		elif not shape_non_null:
			failure_reason = "collision_shape_null"
		elif shape_disabled:
			failure_reason = "collision_shape_disabled"
		elif get_effective_canvas_layer() != 0:
			failure_reason = "wrong_canvas_layer"
		elif z_index < 0:
			failure_reason = "negative_z_index"
		else:
			selection_capable = true
			failure_reason = ""
	else:
		failure_reason = "sprite_missing"
	
	return {
		"pawn_id": int(data.id) if data != null else -1,
		"name": data.display_name if data != null and data.has("display_name") else ("Pawn #" + str(int(data.id) if data != null else -1)),
		"node_path": get_path(),
		"global_position": global_position,
		"sprite_exists": sprite != null and is_instance_valid(sprite),
		"sprite_parent_path": sprite_parent_path,
		"sprite_visible": visible and is_visible_in_tree() and sprite != null and sprite.is_visible_in_tree(),
		"texture_non_null": sprite != null and is_instance_valid(sprite) and sprite.texture != null,
		"click_area_exists": area != null and is_instance_valid(area),
		"click_area_parent_path": click_area_parent_path,
		"input_pickable": area != null and is_instance_valid(area) and area.input_pickable,
		"collision_layer": area.collision_layer if area != null and is_instance_valid(area) else 0,
		"collision_layer_nonzero": area != null and is_instance_valid(area) and area.collision_layer != 0,
		"collision_shape_exists": shape_non_null,
		"collision_shape_disabled": shape_disabled,
		"collision_shape_non_null": shape_non_null,
		"selection_capable": selection_capable,
