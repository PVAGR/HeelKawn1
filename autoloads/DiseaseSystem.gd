...
func add_disease(pawn_data: RefCounted, disease_type: int, initial_severity: float = 20.0, source: String = "unknown") -> void:
	...
	var diseases_val = pawn_data.get("diseases")
	var diseases: Dictionary = diseases_val if diseases_val != null else {}
	...
	if diseases.has(disease_key):
		...
		return
	# New disease
	var params: Dictionary = DISEASE_PARAMS[disease_type] if DISEASE_PARAMS.has(disease_type) else {}
	...
	WorldMemory.record_event({
		...
		"disease": (DISEASE_NAMES[disease_type] if DISEASE_NAMES.has(disease_type) else "unknown"),
		"severity": initial_severity,
		"source": source,
	})
...
func _process_pawn_diseases(pawn: HeelKawnian, tick: int) -> void:
	...
	var _dis_val = data.get("diseases")
	var diseases: Dictionary = {} if _dis_val == null else _dis_val
	...
	for disease_key in diseases:
		...
		if severity <= 0.0:
			...
			WorldMemory.record_event({
				...
				"disease_cured": (DISEASE_NAMES[disease_type] if DISEASE_NAMES.has(disease_type) else "unknown"),
			})
			continue
...
func get_disease_summary(pawn_data: RefCounted) -> Dictionary:
	...
	for disease_key in diseases:
		var disease: Dictionary = diseases[disease_key]
		var disease_type: int = int(disease.get("type", 0))
		result[DISEASE_NAMES[disease_type] if DISEASE_NAMES.has(disease_type) else "unknown"] = {
			"severity": float(disease.get("severity", 0.0)),
			"treated": bool(disease.get("is_treated", false)),
			"contagious": ((DISEASE_PARAMS[disease_type]["contagion"] if DISEASE_PARAMS.has(disease_type) and DISEASE_PARAMS[disease_type].has("contagion") else 0.0) as float) > 0.0,
		}
	return result
