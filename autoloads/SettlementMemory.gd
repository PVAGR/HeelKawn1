...
## center_region -> last festival milestone already celebrated.
var _festival_milestone_by_center: Dictionary = {}
...
func describe_infrastructure_formal_gate(st: Dictionary, center_tile: Vector2i) -> Dictionary:
    ...
    var has_stockpile: bool = _has_local_stockpile(center_rk)
...
func _has_local_stockpile(center_rk: int) -> bool:
    if center_rk < 0 or StockpileManager == null:
        return false
    for zone in StockpileManager.zones():
        if zone == null or not is_instance_valid(zone):
            continue
        if int(zone.settlement_id) == center_rk:
            return true
    return false
    ...
    for i in range(settlements.size()):
        ...
        if allowed:
            ...
            if not was_formal and WorldMemory != null:
                ...
                var narrative: String = "The [b]" + str(polity_nm) + "[/b] confederacy declared itself at " + str(region_lbl) + " after " + str(int(winters)) + " winters of camp-fires and shared bread (" + str(int(gate.get("member_count", 0))) + " souls sworn to the hearth)."
...
## Returns true if the settlement at [param center_region] is active (not abandoned/collapsed).
func is_settlement_active(center_region: int) -> bool:
    var state: String = get_state_at_region(center_region)
    if state.is_empty():
        for st in settlements:
            if st is Dictionary and int(st.get("center_region", -1)) == center_region:
                state = str(st.get("state", "active"))
                break
    return state == "active" or state == "revivable" or state == "recovering"
