...
func _on_game_tick(tick: int) -> void:
	_prune_stale_trade_routes(tick)
...
func _try_create_trade_routes(tick: int) -> void:
	...
	# Get all active settlements (formal only).
...
func _current_formal_settlement_centers() -> Dictionary:
	...
	for st_any in SettlementMemory.get_formal_settlements():
		...
		var state: String = str(st.get("state", ""))
		if state != "active" and state != "revivable":
			continue
		var center: int = int(st.get("center_region", -1))
		if center >= 0:
			centers[center] = true
...
func _prune_stale_trade_routes(tick: int = -1) -> void:
	if SettlementMemory == null or not SettlementMemory.has_method("get_formal_settlements"):
		return
	var formal_centers: Dictionary = _current_formal_settlement_centers()
	if formal_centers.is_empty():
		if not trade_routes.is_empty():
			trade_routes.clear()
			_rebuild_route_caches(tick)
			_update_stats()
		return
	var kept_routes: Array[Dictionary] = []
	var changed: bool = false
	for route_any in trade_routes:
		if not (route_any is Dictionary):
			changed = true
			continue
...
func _update_trade_routes(tick: int) -> void:
	if trade_routes.is_empty():
		return
	var formal_centers: Dictionary = _current_formal_settlement_centers()
	var route_step: float = 1.0 / float(TRADE_ROUTE_DURATION_TICKS)
	for route_index in range(trade_routes.size() - 1, -1, -1):
		var route_any: Variant = trade_routes[route_index]
		if not (route_any is Dictionary):
		...
		if not formal_centers.has(from_settlement) or not formal_centers.has(to_settlement):
			trade_routes.remove_at(route_index)
			continue
		var progress: float = clampf(float(route.get("progress", 0.0)) + route_step, 0.0, 1.0)
		route["progress"] = progress
		route["last_updated_tick"] = tick
		trade_routes[route_index] = route
		if progress < 1.0:
			continue
		_deliver_route_goods(route)
		_spread_knowledge(from_settlement, to_settlement, route.get("goods", {}) as Dictionary)
		var goods_total: int = 0
		for value in (route.get("goods", {}) as Dictionary).values():
		...
		if OS.is_debug_build():
			print("[TradeMemory] Route %d completed: %d → %d" % [
				int(route.get("route_id", -1)), from_settlement, to_settlement
			])
		trade_routes.remove_at(route_index)
		stats.completed_routes += 1
		stats.total_goods_traded += goods_total
	_rebuild_route_caches(tick)
...
func _deliver_route_goods(route: Dictionary) -> void:
	...
	for key in goods:
		...
		# Determine how many are available across all stockpiles and remove them
		var available: int = StockpileManager.total_count_of(item_type)
		var to_move: int = mini(qty, available)
		if to_move <= 0:
			continue
		StockpileManager.remove_item(item_type, to_move)

		# Deposit into destination zone (prefer settlement-local)
		...
		if zone != null:
			zone.add_item(item_type, to_move)
